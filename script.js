...
function showElevationChart(data, geoLayer) {
  ...
  const distance = data.map(p => p.FIRST_DIST / 1000);
  ...
  elevationChart = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        ...
        fill: true,
        tension: 0.25,
        pointRadius: 3, // aby byl hover přesnější
      }]
    },
    options: {
      responsive: true,
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: ctx => `${ctx.parsed.y.toFixed(1)} m n. m.`
          }
        },
        title: {
          display: true,
          text: 'Výškový profil trati (km / m n. m.)'
        }
      },
...
// --- Funkce pro marker na mapě ---
function updateMapMarker(index, geoLayer) {
  if (!geoLayer.latlngs) return;
  const point = geoLayer.latlngs[index];
  if (!point) return;

  if (!elevationMarker) {
    elevationMarker = L.circleMarker(point, { radius: 6, color: '#ff2a00', fillColor: '#ff2a00', fillOpacity: 1 }).addTo(map);
  } else {
